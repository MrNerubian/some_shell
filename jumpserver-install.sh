...
echo -e "\033[31m 2. 部署环境 \033[0m" \
  && wget https://www.nerubian.cn/yum/tuna.repo ; mv tuna.repo /etc/yum.repos.d \
  && yum update -y \
...
echo -e "\033[31m 3. 下载组件 \033[0m" \
  ...
  && if [ ! -f "/opt/py3/bin/activate" ]; then yum -y install python36 python36-devel; fi \
...
echo -e "\033[31m 5. 启动 Jumpserver \033[0m" \
  ...
  && echo 你的数据库密码是 $DB_PASSWORD > /root/jumpserver.txt \
  && echo 你的'SECRET_KEY'是 $SECRET_KEY >> /root/jumpserver.txt \
  && echo 你的BOOTSTRAP_TOKEN是 $BOOTSTRAP_TOKEN >> /root/jumpserver.txt \
  && echo 你的服务器IP是 $Server_IP >> /root/jumpserver.txt \
  && echo 请打开浏览器访问 http://$Server_IP 用户名:admin 密码:admin >> /root/jumpserver.txt \
  && cat /root/jumpserver.txt \
